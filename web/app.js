...
// read from the last `bun run eval`. hard-coding these would let the dashboard
// quote numbers the code no longer produces.
let EVAL = { precision: 1.0, recall: 1.0, f1: 1.0, fpr: 0.0, cases: 15, generatedAt: null };
...
async function loadTrust() {
  ...
  const stamp = $("evalStamp");
  if (stamp) {
    stamp.textContent = EVAL.generatedAt
      ? `golden set, ${EVAL.cases} cases · last run ${new Date(EVAL.generatedAt).toLocaleString()}`
      : `golden set, ${EVAL.cases} cases`;
  }
